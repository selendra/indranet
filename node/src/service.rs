...
use forests_client_consensus_common::{ParachainBlockImport, ParachainConsensus};
use forests_client_consensus_relay_chain::Verifier as RelayChainVerifier;
use forests_client_network::BlockAnnounceValidator;
use forests_client_service::{
...
pub mod indranet {
	pub use indranet_runtime::RuntimeApi;

	/// Indranet runtime executor.
	pub struct Executor;
	impl sc_executor::NativeExecutionDispatch for Executor {
		...
		fn dispatch(method: &str, data: &[u8]) -> Option<Vec<u8>> {
			indranet_runtime::api::dispatch(method, data)
		}

		fn native_version() -> sc_executor::NativeVersion {
			indranet_runtime::native_version()
		}
	}
}
	...
	Executor: sc_executor::NativeExecutionDispatch + 'static,
{
	...
	let aura_verifier = move || {
		let slot_duration = forests_client_consensus_aura::slot_duration(&*client2).unwrap();

		Box::new(forests_client_consensus_aura::build_verifier::<
...
/// Start a parachain node for Indranet.
pub async fn start_indranet_node(
	parachain_config: Configuration,
	selendra_config: Configuration,
	collator_options: CollatorOptions,
	id: ParaId,
	...
	TaskManager,
	Arc<TFullClient<Block, indranet::RuntimeApi, NativeElseWasmExecutor<indranet::Executor>>>,
)> {
	start_contracts_node_impl::<indranet::RuntimeApi, indranet::Executor, _, _>(
        parachain_config,
        selendra_config,
        collator_options,
        id,
        ...
        |client,
         ...
         force_authoring| {
            ...
            let aura_consensus = BuildOnAccess::Uninitialized(Some(
                Box::new(move || {
                    let slot_duration =
                        forests_client_consensus_aura::slot_duration(&*client2).unwrap();
                            ...
                            move |_, (relay_parent, validation_data)| {
                                ...
                                async move {
                                    let parachain_inherent =
                                        forests_primitives_parachain_inherent::ParachainInherentData::create_at(
                                            relay_parent,
            ...
            let relay_chain_consensus =
                forests_client_consensus_relay_chain::build_relay_chain_consensus(
                    forests_client_consensus_relay_chain::BuildRelayChainConsensusParams {
                        para_id: id,
                        proposer_factory,
