...
impl EvmData for Junction {
	fn read(reader: &mut EvmDataReader) -> EvmResult<Self> {
		...
		ensure!(!junction_bytes.is_empty(), revert("Junctions cannot be empty"));
...
impl EvmData for Junctions {
	fn read(reader: &mut EvmDataReader) -> EvmResult<Self> {
		...
		for item in junctions_bytes {
			junctions.push(item).map_err(|_| revert("overflow when reading junctions"))?;
		}
