...
use frame_support::{assert_err, assert_ok, parameter_types, traits::Contains};
use hex_literal::hex;
...
fn new_test_ext() -> TestExternalities {
	...
	let _ = pallet_balances::GenesisConfig::<Runtime> { balances: vec![(account, 1_000_000_000)] }
...
fn invalid_signature() {
	...
	let call =
		pallet_balances::Call::<Runtime>::transfer { dest: alice.clone(), value: 1_000 }.into();
...
fn not_allowed_call_filtered() {
	new_test_ext().execute_with(|| {
		...
		let call: RuntimeCall =
			frame_system::Call::<Runtime>::remark { remark: Vec::<_>::new() }.into();
