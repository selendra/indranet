...
fn only_root_as_origin() {
	ExternalityBuilder::build().execute_with(|| {
		...
		assert_noop!(XcAssetConfig::remove_asset(RuntimeOrigin::signed(1), asset_id,), BadOrigin);
	})
}
...
fn remove_payment_asset_is_ok() {
	ExternalityBuilder::build().execute_with(|| {
		...
		System::assert_last_event(mock::RuntimeEvent::XcAssetConfig(
			Event::SupportedAssetRemoved { asset_location: asset_location.clone().versioned() },
		));
		assert!(!AssetLocationUnitsPerSecond::<Test>::contains_key(
			asset_location.clone().versioned()
		));
...
fn remove_asset_is_ok() {
	ExternalityBuilder::build().execute_with(|| {
		...
		assert!(!AssetLocationToId::<Test>::contains_key(asset_location.clone().versioned()));
		assert!(!AssetLocationUnitsPerSecond::<Test>::contains_key(
			asset_location.clone().versioned()
		));
	})
...
fn public_interfaces_are_ok() {
	ExternalityBuilder::build().execute_with(|| {
		...
		assert_eq!(XcAssetConfig::get_xc_asset_location(asset_id), Some(asset_location.clone()));
		assert_eq!(XcAssetConfig::get_asset_id(asset_location.clone()), Some(asset_id));
		assert!(XcAssetConfig::get_units_per_second(asset_location.clone()).is_none());
